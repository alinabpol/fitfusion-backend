...
def get_one_recipe(id):
    recipe = models.Snacks.get_by_id(id)
    print(recipe)
    return jsonify(
...
def create_recipe():
    ...
    print(payload)
    new_recipe = models.Snacks.create(title=payload['title'], img=payload['img'], time=payload["time"], ingredients=payload["ingredients"], description=payload['description'])
...
def update_recipe(id):
    ...
    models.Snacks.update(**payload).where(models.Snacks.id == id).execute()

    return jsonify(
        data=model_to_dict(models.Snacks.get_by_id(id)),
...
def delete_recipe(id):
    ...
    delete_query = models.Snacks.delete().where(models.Snacks.id == id)
