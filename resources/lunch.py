...
def get_one_recipe(id):
    recipe = models.Lunch.get_by_id(id)
    print(recipe)
    return jsonify(
...
def create_recipe():
    ...
    print(payload)
    new_recipe = models.Lunch.create(title=payload['title'], img=payload['img'], time=payload["time"], ingredients=payload["ingredients"], description=payload['description'])
...
def update_recipe(id):
    ...
    models.Lunch.update(**payload).where(models.Lunch.id == id).execute()

    return jsonify(
        data=model_to_dict(models.Lunch.get_by_id(id)),
...
def delete_recipe(id):
    ...
    delete_query = models.Lunch.delete().where(models.Lunch.id == id)
