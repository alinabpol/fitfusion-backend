...
collection = smoothies_collection
...
def nutrition_index():
    ...
    nutrition_dicts =  [
        ...
        {**smoothies, '_id': str(smoothies['_id'])} for smoothies in result
    ]
...
def get_one_recipe(id):
    smoothies = collection.find_one({'_id': ObjectId(id)})
    if smoothies:

        smoothies['_id'] = str(smoothies['_id'])
...
def create_recipe():
    ...
    smoothies = Smoothies(**payload)
    result = smoothies.save()
    if result:
        return jsonify({
            'data': smoothies.__dict__,
