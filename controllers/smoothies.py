...
def nutrition_index():
    result = nutrition_models.Smoothies.select()
...
def get_one_recipe(id):
    recipe = nutrition_models.Smoothies.get_by_id(id)
    print(recipe)
    return jsonify(
...
def create_recipe():
    ...
    print(payload)
    new_recipe = nutrition_models.Smoothies.create(title=payload['title'], img=payload['img'], time=payload["time"], ingredients=payload["ingredients"], description=payload['description'])
...
def update_recipe(id):
    ...
    nutrition_models.Smoothies.update(**payload).where(nutrition_models.Smoothies.id == id).execute()

    return jsonify(
        data=model_to_dict(nutrition_models.Smoothies.get_by_id(id)),
...
def delete_recipe(id):
    ...
    delete_query = nutrition_models.Smoothies.delete().where(nutrition_models.Smoothies.id == id)
